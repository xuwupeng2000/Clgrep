...
int* SetHorspoolPrecomputation(string *needles, int characterSetSize, int shortestNeedleLen, int needleNum) {
    ...
    for(i=0; i<characterSetSize; i++) {
        skipTable[i] = shortestNeedleLen;//If one char is not a part of the needle then skip as far as the shortest needle length
    }
    for(i=0; i<needleNum; i++) {
        char *needlePtr = needles[i];
        int needleLen = strlen(needlePtr);
        int j;
        int temp =needleLen-1;
        for(j=0; j<temp; j++) {
            int shift = needleLen-j-1;
            if(shift <= shortestNeedleLen) {//If the shift value even bigger than the shortestNeedleLen then I do not need to compare it anymore
...
int main(int argc, char * argv[]) {
    ...
    char *kernelFile = "./kernel/SH_GPU_LocalMem_OpenCL_Kernel.cl";//The kernel file's name
    ...
    //Create the kernel
    cl_kernel kernel = clCreateKernel(program, "SetHorspoolMatch", &err);
    if(err != CL_SUCCESS) {
    ...
    err |= clSetKernelArg(kernel, 10, sizeof(cl_mem), &lastPosOfEachNeedleBuffer);
    //~ err |= clSetKernelArg(kernel, 11, sizeof(int)*needleNum, NULL);
    err |= clSetKernelArg(kernel, 11, sizeof(cl_mem), &lenOfEachNeedleBuffer);
    //~ err |= clSetKernelArg(kernel, 13, sizeof(cl_mem), NULL);//localLEN
    err |= clSetKernelArg(kernel, 12, sizeof(cl_mem), &resBuffer);
    if(err != CL_SUCCESS) {
